...
void loop() {
  ...
  if (currentMicros - lastSampleTime >= intervalMicros) {
    ...
    // Simple Rectangular Window Envelope (Moving Average)
    float emgRect_Env = rectSum / windowLength;
    ...
    if (plotCounter >= plotDecimation) {
      plotCounter = 0;
      Serial.print("Rectified_Signal:");
      Serial.print(emgRectified);
      Serial.print(",");
      Serial.print("Rectwin_Envelope:");
      Serial.print(emgRect_Env);
      Serial.print(",");
      Serial.print("Hanning_Envelope:");
      Serial.println(emgHann_Env);
    }
  }
}
